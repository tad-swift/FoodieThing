    ...
    @objc func pinched(recognizer: UIPinchGestureRecognizer) {
           if recognizer.state == .changed {
               ...
               if let hitTest = hitTestResults.first {
                   let objectNode = hitTest.node
                   let pinchScaleX = Float(recognizer.scale) * objectNode.scale.x
                   let pinchScaleY = Float(recognizer.scale) * objectNode.scale.y
                   let pinchScaleZ = Float(recognizer.scale) * objectNode.scale.z
                   objectNode.scale = SCNVector3(pinchScaleX,pinchScaleY,pinchScaleZ)
                   recognizer.scale = 1
               }
           }
       }
